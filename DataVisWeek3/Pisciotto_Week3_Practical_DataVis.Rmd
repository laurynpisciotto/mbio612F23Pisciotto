...
```{r}
# Write your code here---ORIGINAL CODE
ggplot(data = mpg) +
    geom_point(mapping = aes(x = displ, y = hwy, color = "blue"))
```
...
#to turn the points blue the colour = blue needs to be outside of the aes 
```
...
```{r}
# Write your code here
# i'm not sure how to use the tilde for this but from using the help function I was able to do it another way i think
ggplot(data = mpg) +
    geom_point(mapping = aes(x = displ, y = hwy)) +
    facet_grid(rows = vars(class), vars(trans))
```


```{r}
##from redoing in class was able to get it using the ~
...
```{r}
# Write your code here
# I changed the size so its easier to see
ggplot(data = mpg) + 
...
#im not sure if I omitted the outliers correctly
ggplot(sizeMass, aes(size, mass)) +
      geom_point() +
      geom_smooth(method = "lm", color = "red", se = FALSE) +
      xlim(c(0,1000)) +
...
mass_log10 <- log(mass)
sizeMassLog <- data.frame(mass, size, mass_log10, size_log10)
